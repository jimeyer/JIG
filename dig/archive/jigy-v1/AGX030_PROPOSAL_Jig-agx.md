---
title: "JIG Agent Extensions (agx) Architecture"
type: exploration
status: implemented
...
The build command reads intents, validates them, renders templates, and copies supporting files.
...
AGX_ROOT = Path(__file__).parent.parent.parent / 'agx'

@click.group()
def cli():
    """JIG - Just-In-Time Governance CLI"""
    pass

@cli.command()
@click.option('--intent', '-i', help='Build specific intent only')
@click.option('--dry-run', is_flag=True, help='Show what would be built')
def build(intent, dry_run):
    """Generate Claude skills and Cursor rules from intents."""
    ...
    if intent:
        intent_files = [intents_dir / f'{intent}.yaml']
    else:
        intent_files = list(intents_dir.glob('*.yaml'))
    
    for intent_path in intent_files:
        if not intent_path.exists():
            click.echo(f"Intent not found: {intent_path}", err=True)
            continue
...
@cli.command()
@click.argument('target', type=click.Choice(['claude', 'cursor', 'both']))
@click.option('--project', '-p', is_flag=True, help='Install to current project instead of user config')
def init(target, project):
    """Install jig agent instructions to editor config locations."""
    ...
    if target in ('claude', 'both'):
        ...
        click.echo(f"✓ Claude skills installed to {dest}")
    
    if target in ('cursor', 'both'):
        if project:
            dest = Path.cwd() / '.cursor' / 'rules' / 'jig'
        else:
            click.echo("Cursor only supports project-level rules", err=True)
            return
        ...
        click.echo(f"✓ Cursor rules installed to {dest}")

@cli.command()
def validate():
    """Validate all intent files against schema."""
...
Only the generated artifacts are distributed to users:
...
# Note: agx/authoring/ is excluded from distribution
```
...
```bash
# When jig is updated
pip install --upgrade jig

# Re-deploy to pick up new/changed instructions
jig init both --project
```
...
jig eval -i ostc-alignment

# Run only activation tests
jig eval --tag activation

# Run with verbose output
jig eval -v

# Generate eval report
jig eval --report eval-results.json
```

### Eval Implementation Sketch

```python
@cli.command()
@click.option('--intent', '-i', help='Eval specific instruction only')
@click.option('--tag', '-t', multiple=True, help='Filter by tag')
@click.option('--report', type=click.Path(), help='Output JSON report')
def eval(intent, tag, report):
    """Run evaluation cases against agent instructions."""
    ...
    for eval_dir in evals_dir.iterdir():
        if not eval_dir.is_dir():
            continue
        if intent and eval_dir.name != intent:
            continue
        ...
        for case in cases['cases']:
            if tag and not any(t in case.get('tags', []) for t in tag):
                continue
            ...
            click.echo(f"{status} {cases['name']}/{case['id']}")
...
jobs:
  eval:
    ...
    steps:
      ...
      - name: Run evals
        run: jig eval --report eval-results.json
...
# 3. Rebuild
jig build -i existing-instruction
...
# 6. Run evals
jig eval -i existing-instruction
...
If Anthropic expands their skills marketplace, jig could publish skills there directly:

```bash
jig publish --platform anthropic
```
...
3. **Clean separation** — Authoring inputs stay in development; only generated artifacts ship to users
4. **Testability** — Evals verify instructions work correctly before deployment
5. **Simple workflow** — `jig build` to generate, `jig init` to deploy, `jig eval` to verify

The `agx/` directory sits alongside `src/` as a first-class part of the jig project, treating agent instructions with the same rigor as code.
