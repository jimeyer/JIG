...
JIG tracks three edge types in the alignment graph:
...
| Document | Role |
|----------|------|
| **J022** | Computes `jig_hash` for S, F, T nodes |
...
|-------|------|----------|-------------|
| `edge` | enum | Yes | Edge type: `F→S`, `T→S`, `T→F` |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `id` | string | Yes | Node identifier (F-xxx, T-xxx, S-xxx) |
...
```
jigy rebuild
    └─→ Computes current jig_hash for S, F, T (from J022)
...
def detect_audit_triggers(
    ...
    Args:
        intent_graph: Current intent graph with spec jig_hash values
...
Summary: 9 triggers (5 new, 2 from_changed, 1 to_changed, 1 removed)
```
...
│                                                                 │
│  1. DECLARE                                                     │
│     Developer writes specs, decorators, tests                   │
│                        ↓                                        │
│  2. REBUILD                                                     │
...
**Field contract:**
- `edge` (enum, REQUIRED): Edge type (`F→S`, `T→S`, `T→F`)
...
5. **Auditor diff workflow** — `git_commit` enables easy diff commands
6. **Trend analysis** — track alignment health over time
7. **Team coordination** — shared audit state across developers
8. **CI integration** — automated detection of unaudited changes
